...
ssp5 <- read.csv("D:/Three anona/uvaria.csv")
...
training2 <- training %>% select(-c('bio6','bio4', 'bio10','ai_v3_yr', 'bio11', 'bio1', 
                                    'bio2', 'bio18', 'bio19', 'bio15', 'bio16'))
...
training2 <- training3 %>% mutate(species = case_when(species == "Uvaria kirkii Oliv" ~ "Uvaria")) %>% 
...
d2
m4 <- sdm(Uvaria~.,d2, methods =c("rf","MAXENT"),
          replication=c('sub','boot'),test.p=30,n=10)
m4
...
roc(m4, species = "Uvariodendron", smooth = T)
...
enm <- ensemble(m4,biom,'Uvarimkcu.img',setting = list(id=1:40,method='weighted',stat="AUC"))
...
#biof4570 <- raster::getData("CMIP5", var='bio', res=2.5, rcp=45, model='AC', year=50)
#biof4570 <- raster::getData("CMIP5", var='bio', res=2.5, rcp=45, model='AC', year=70)
#biof4550 <- raster::getData("CMIP5", var='bio', res=2.5, rcp=85, model='AC', year=50)
#biof4550 <- raster::getData("CMIP5", var='bio', res=2.5, rcp=85, model='AC', year=70)
#getwd()
#names (biof4550)

biof4550 <- stack(list.files(path = "D:/Three anona/ACCESS/ACCESS/uvaria4550asci",pattern = ".asc$",full.names = TRUE))
...
enmf <- ensemble(m4,biof4550,'ovaria.img',setting = list(id= 1:40,method='weighted',stat='AUC',opt=2))
...
writeRaster(ch,"new_mkalac2.asc",format ="ascii")

read.asciigrid("new_mkalac2.asc") -> ch1

plot(ch1)

getEvaluation(m5,stat ='AUC')

ev <- getEvaluation(m4,stat = c('AUC','TSS', 'threshold'),opt = 2)

mean(ev$threshold)

pa <- raster(enm)

pa[] <- ifelse(enm[] >=    0.1655318, 1, 0)

plot(pa)

paf <- raster(enmf)

paf[] <- ifelse(enmf[] >=   0.1655318, 1, 0)
...
niche(biom,enm,n=c('bio7','bio17'),col=cl(200))
